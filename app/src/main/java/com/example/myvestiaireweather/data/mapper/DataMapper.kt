...
internal fun WeatherResponse.toWeatherCache() = WeatherCacheModel(
    city = CityCacheModel( country =  city.country, name = city.name),
    cnt= cnt,
    weatherData = list.map { it.weatherDataCache() }
)

private fun Weather.weatherDataCache() = WeatherDataCacheModel(
    clouds = clouds,
    degree = deg,
    timeOfForecast = dt,
    feels_like =
    ...
    speed = speed,
    sunrise = sunrise,
    temp = Temp(day = temp.day, eve = temp.day, max = temp.max,min = temp.min, morn = temp.morn , night = temp.night),
    weatherInfo = weatherInfo.map { it.weatherInfoCache() },

    )

    private fun WeatherInfo.weatherInfoCache() = WeatherInfoCacheModel(
        id = id,
        main = main,
...
/** cache to domain **/
internal fun WeatherCacheModel.toWeatherEntity() = WeatherEntity(
    city = City( country =  city.country, name = city.name),
    cnt= cnt,
    list = weatherData.map { it.weatherDataEntity() }
)

private fun WeatherDataCacheModel.weatherDataEntity() = WeatherDataEntity(
    clouds = clouds,
    degree = degree,
    timeOfForecast = timeOfForecast,
    feelLike =FeelsLikeEntity(
        day = feels_like.day,
...
private fun WeatherInfoCacheModel.weatherInfoEntity() = WeatherInfoEntity(
    id = id,
    main = main,
    description = description,
    icon = icon
